...
# ==== Route A：in-script 环境钩子，必须早于任何 torch 的导入 ====
# 1) 关闭 torch.compile / Dynamo / Inductor（让 @torch.compile 变成 no-op）
os.environ.setdefault("TORCHDYNAMO_DISABLE", "1")
os.environ.setdefault("TORCHINDUCTOR_DISABLE", "1")

# 2) 防止 CUDA 串味：优先本环境的 torch/cuda 动态库，剔除 /usr/local/cuda-*
...
torch.serialization.add_safe_globals(
    [ListConfig, DictConfig, ContainerMetadata, Any, Metadata, list, defaultdict, dict, int,
     AnyNode, ])  # 允许 ListConfig 反序列化
...
class SequenceLightningModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.save_hyperparameters(config, logger=False)  # 将所有配置信息存为 self.hparams，可自动保存到日志和 checkpoint

        # Dataset arguments
        self.dataset = SequenceDataset.registry[self.hparams.dataset._name_](
            **self.hparams.dataset  # 实例化  把 config 里的其它字段（data_dir, batch_size...）作为参数传入，生成数据集对象
        )
    ...
    def setup(self, stage=None):
        ...
        # Convenience feature: if model specifies encoder, combine it with main encoder [None,None]
        encoder_cfg = utils.to_list(self.hparams.encoder) + utils.to_list(
            self.hparams.model.pop("encoder", None)
        )
        # print('encoder_cfg:', encoder_cfg)  # [None,None]
        decoder_cfg = utils.to_list(
            self.hparams.model.pop("decoder", None)
        ) + utils.to_list(self.hparams.decoder)
        # print('decoder_cfg:', decoder_cfg)  # # [None,None]

        # Instantiate model
        self.model = utils.instantiate(registry.model, self.hparams.model)  # 根据参数指定模型并进行参数化
        print('self.model:', self.model)  # 输出模型的结构
        ...
        # print('self.encoder:', self.encoder)  # Identity()
        self.decoder = U.PassthroughSequential(decoder, self.task.decoder)
        # print('self.decoder:', self.decoder)  # Identity()
    ...
    def load_state_dict(self, state_dict, strict=False):
        ...
        # state_dict = torch.load("/mnt/nas/share2/home/by/hyena-dna/outputs/2024-05-11/23-28-32-918372/checkpoints/val/mcc.ckpt")['state_dict']
    ...
    #             print(name, None if param.grad is None else param.grad.abs().mean().item())

    def training_step(self, batch, batch_idx, dataloader_idx=0):

        # print('batch:', batch)
        # print("batch len:", len(batch))
        # print("inputs shape:", batch[0].shape)
        ...
        #     if param.requires_grad:
        #         print('检查参数更新')
        #         print(name, param.data.view(-1)[0].item())
        #         break

        loss = self._shared_step(batch, batch_idx, prefix="train")
        # print('loss in training_step', loss, "loss requires_grad:", loss.requires_grad)


        # print('检查loss.backward 之后梯度')
        # for name, param in self.model.named_parameters():
        #     if param.requires_grad:
        ...
        #          on_step=True,    # 不在每个 batch 上都打
        #          on_epoch=True,    # 而是在每个 epoch 末打一次
        #          prog_bar=True,    # 是否在进度条显示
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        import csv
        preds = x.argmax(dim=-1)  # 假设是分类任务
        incorrect_indices = (preds != y.flatten()).nonzero(as_tuple=True)[0]  # 找到分类错误的样本索引

        incorrect_sequences = [batch[0][i] for i in incorrect_indices]  # 提取错误的数字序列
        incorrect_labels = y.flatten()[incorrect_indices].tolist()
        incorrect_preds = preds[incorrect_indices].tolist()

        # 定义数字到字符的映射表
        idx_to_char = {0: 'A', 1: 'C', 2: 'G', 3: 'T', 4: 'N'}

        # 将数字序列映射回字符序列
        incorrect_char_sequences = [
            ''.join(idx_to_char[idx] if idx in idx_to_char else "N" for idx in sequence.tolist())
            # sequence是Tensor，需转换为列表
            for sequence in incorrect_sequences
        ]

        # 获取当前时间和阶段信息
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        marker = f"test-{current_time}"  # 使用 prefix 和时间标记

        # 去重：仅保存未出现过的序列
        new_data = []
        for char_seq, true_label, pred_label in zip(incorrect_char_sequences, incorrect_labels, incorrect_preds):
            if char_seq not in saved_sequences:
                new_data.append(
                    {"sequence": char_seq, "true_label": true_label, "pred_label": pred_label, "marker": marker})
                saved_sequences.add(char_seq)

        # 将错误样本追加保存到CSV文件
        if new_data:  # 如果有新数据才写入
            output_path = "/mnt/nas/share2/home/by/hyena-dna/incorrect_sequences_cnn.csv"
            write_header = not os.path.exists(output_path)  # 仅首次写入时添加表头

            with open(output_path, mode='a', newline='', encoding='utf-8') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=["sequence", "true_label", "pred_label", "marker"])
                if write_header:
                    writer.writeheader()

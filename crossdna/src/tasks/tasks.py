    ...
    def _init_torchmetrics(self):
        ...
        for name in self.torchmetric_names:
            if name in tm_mine:
                ...
                if name == "F1":
                    name = "F1Score"
                tracked_torchmetrics[name] = getattr(tm, name)(average='macro', num_classes=self.dataset.d_output,
                                                               compute_on_step=False, task=self.dataset.task)
            elif '@' in name:
                k = int(name.split('@')[1])
                mname = name.split('@')[0]
                tracked_torchmetrics[name] = getattr(tm, mname)(average='macro', num_classes=self.dataset.d_output,
                                                                compute_on_step=False, top_k=k)
            else:
...
class LMTask(BaseTask):
    def forward(self, batch, encoder, model, decoder, _state):
        """Passes a batch through the encoder, backbone, and decoder"""
        # print('HG38 预训练时候使用了这个LMTaskd的forward函数，微调GB的时候没有使用')
        # z holds arguments such as sequence length
        x, y, *z = batch  # z holds extra dataloader info such as resolution
        if len(z) == 0:
            z = {}
...
class MaskedMultiClass(MultiClass):
    ...
    def forward(self, batch, encoder, model, decoder, _state):
        ...
        x, w = encoder(
            x)  # w can model-specific constructions such as key_padding_mask for transformers or state for RNNs
        x, state = model(x)
        self._state = state
        x, w = decoder(x, state=state, **z)
        return x, y, w

...
class HG38(SequenceDataset):
    ...
    def __init__(self, bed_file, fasta_file, tokenizer_name=None, dataset_config_name=None, max_length=1024, d_output=2,
        ...
        if self.bed_file is None:
            ...
            print('self.bed_file:',self.bed_file)
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in HG38**")
        ...
        elif self.tokenizer_name == "ntv2-kmer":
            print("**using pretrained InstaDeepAI tokenizer**")
            self.tokenizer = AutoTokenizer.from_pretrained("InstaDeepAI/nucleotide-transformer-v2-500m-multi-species",
...
class GenomicBenchmark(HG38):
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in GB**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                # characters=['A', 'C', 'G', 'T', 'P', 'Q', 'N'], # 增加甲基化的点位token
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
            )
        elif self.tokenizer_name == 'bpe':
...
class NucleotideTransformer(HG38):
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in NT**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
...
class ChromatinProfile(HG38):
    ...
    def setup(self, stage=None):
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in ChromatinProfile**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
        ...
        self.feature_names = self.dataset_train.feature_names

class Species(HG38):
    _name_ = "species"
    l_output = 0  # need to set this for decoder to work correctly

    def __init__(self, species: list, species_dir: str, tokenizer_name=None, dataset_config_name=None, d_output=None,
                 max_length=1024, rc_aug=False,
                 max_length_val=None, max_length_test=None, cache_dir=None, val_ratio=0.0005, val_split_seed=2357,
                 add_eos=True, detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
                 shuffle=False, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                 fast_forward_epochs=None, fast_forward_batches=None, chromosome_weights='uniform',
                 species_weights='uniform',
                 total_size=None, task='species_classification', remove_tail_ends=False, cutoff_train=0.1,
                 cutoff_test=0.2,
                 *args, **kwargs):
        self.dataset_config_name = dataset_config_name
        self.tokenizer_name = tokenizer_name
    ...
    def setup(self, stage=None):
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in Species**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            SpeciesDataset(species=self.species,
                           ...
                           total_size=self.total_size * (
                               1 if split == 'test' else (self.max_length_test + 2) // max_len),
                           # See the same # of tokens every epoch across train/val/test
                           tokenizer=self.tokenizer,  # pass the tokenize wrapper
                           tokenizer_name=self.tokenizer_name,
                           add_eos=self.add_eos,
                           rc_aug=self.rc_aug,
...
class ICLGenomics(HG38):
    ...
    def __init__(self, dataset_name, dest_path=None, tokenizer_name='char', d_output=None, rc_aug=False,
                 max_length=1024, use_padding=True, max_length_val=None, max_length_test=None, shots=1,
                 label_to_token=None,
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in ICLGenomics**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
...
class BertHG38(SequenceDataset):
    ...
    def __init__(self, bed_file, fasta_file, tokenizer_name=None, dataset_config_name=None, max_length=1024, d_output=2,
                 ...
                 shuffle=False, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                 fast_forward_epochs=None, fast_forward_batches=None, replace_N_token=False, pad_interval=False,
                 use_tokenizer=True, pad_max_length=None,
                 objective="stdmlm",
                 *args, **kwargs):
        self.dataset_config_name = dataset_config_name
        self.tokenizer_name = tokenizer_name
        ...
        if self.fasta_file is None:
            self.fasta_file = default_data_path / self._name_ / 'hg38.ml.fa'
            # self.fasta_file = default_data_path / self._name_ / 'hg38.ml.cpg_replaced.fa'  #  加入甲基化位点的hg38数据
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in BertHG38**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                # characters=['A', 'C', 'G', 'T', 'P', 'Q', 'N'],  # 增加甲基化的token
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
            )
        elif self.tokenizer_name == 'bpe':
    ...
    def init_datasets(self):
        ...
        if self.use_fixed_len_val:  # 默认设置为fasle
            # we're placing the fixed test set in the val dataloader, for visualization!!!
            # that means we should track mode with test loss, not val loss
...
class DNABert2Pretrain(SequenceDataset):
    ...
    def __init__(self, text_file, tokenizer_name=None, dataset_config_name=None, max_length=1024, d_output=2,
                 rc_aug=False,
                 max_length_val=None, max_length_test=None, val_ratio=0.0005, val_split_seed=2357,
                 use_fixed_len_val=False,
                 add_eos=True, detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in DNABert2Pretrain**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
    ...
    def init_datasets(self):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            DNABERT2Dataset(split=split,
                            ...
                            objective=self.objective)
            for split, max_len in
            zip(['train', 'val', "test"], [self.max_length, self.max_length_val, self.max_length_test])
        ]
...
class DNABertSPretrain(SequenceDataset):
    ...
    def __init__(self, text_file, seq_name='seq_a', tokenizer_name=None, dataset_config_name=None, max_length=1024,
                 d_output=2, rc_aug=False,
                 max_length_val=None, max_length_test=None, val_ratio=0.0005, val_split_seed=2357,
                 add_eos=True, detokenize=False, val_only=False, batch_size=32, batch_size_eval=None, num_workers=1,
                 shuffle=False, pin_memory=False, drop_last=False, fault_tolerant=False, ddp=False,
                 fast_forward_epochs=None, fast_forward_batches=None, replace_N_token=False, pad_interval=False,
    ...
    def setup(self, stage=None):
        ...
        if self.tokenizer_name == 'char':
            print("**Using Char-level tokenizer in DNABertSPretrain**")
            self.tokenizer = CharacterTokenizer(
                characters=['A', 'C', 'G', 'T', 'N'],
                model_max_length=self.max_length + 2,  # add 2 since default adds eos/eos tokens, crop later
                add_special_tokens=False,
    ...
    def init_datasets(self):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            DNABERTSDataset(split=split,
                            ...
                            objective=self.objective)
            for split, max_len in
            zip(['train', 'val', "val"], [self.max_length, self.max_length_val, self.max_length_test])
        ]

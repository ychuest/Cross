...
class DefaultCollateMixin:
    ...
    CollateMixin 类通过将 collate 参数与其他 dataloader 参数分开，来实例化一个 dataloader。
    该类的实例可以根据需要修改回调函数，并调整 collate_args 列表。
    然后，该类定义了一个 _dataloader() 方法，该方法接收一个 DataLoader 构造函数及其参数，根据 collate_args 构建 collate_fn，并将其余参数传递给构造函数进行实例化。
...
class SequenceDataset(DefaultCollateMixin):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        cls.registry[cls._name_] = cls # 用 _name_ 作为key，把这个子类的类型（class对象）注册到 registry 里
